...
import { StreakWeekDayEntry } from "@/hooks/useStreaks";
...
interface EditPrayerModalProps {
  ...
  selectedDay: StreakWeekDayEntry | null;
  refetch: () => void;
}
...
const EditPrayerModal = ({
  ...
  useEffect(() => {
    ...
    PRAYERS.forEach((p) => {
      initial[p.toLowerCase()] =
        selectedDay?.data?.prayers?.[p.toLowerCase()]?.isPrayed || false;
    });
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      PRAYERS.forEach((prayer) => {
        ...
        const oldStatus =
          selectedDay.data?.prayers?.[prayerKey]?.isPrayed || false;

        if (newStatus !== oldStatus) {
          updateData[`prayers.${prayerKey}.isPrayed`] = newStatus;
          updateData[`prayers.${prayerKey}.status`] = newStatus
            ? "completed"
            : null;
          updateData[`prayers.${prayerKey}.completedAt`] = newStatus
            ? serverTimestamp()
            : null;
      ...
      const newCompletedCount = Object.values(localPrayers).filter(
        (status) => status,
      ).length;
      ...
      if (Object.keys(updateData).length > 0) {
        try {
          ...
          if (errorCode === "not-found") {
            ...
            const newCount = Object.values(localPrayers).filter(
              (status) => status,
            ).length;
            ...
            queryClient.invalidateQueries({
              queryKey: ["dashboard", user?.profile?.uid],
            });
          } else {
            throw err;
          }
      ...
      <View className="flex-1 bg-emerald-darkest">
        ...
        <View className="flex-row justify-between items-center px-6 pt-14 pb-6 border-b border-white/5">
          ...
          <View className="items-center flex-1">
            <Text
              className="text-gold text-xl font-bold text-center"
        ...
        <ScrollView className="flex-1 px-6 pt-8">
          <Text className="text-white/40 text-[10px] font-bold uppercase tracking-widest text-center mb-2">
            Weekly Progress
          </Text>
          <Text className="text-white/60 text-sm text-center mb-8 px-4">
            Update your prayer records for{" "}
          ...
          <View className="space-y-4">
            {PRAYERS.map((prayer) => {
              ...
              const prayerTime =
                selectedDay.data?.prayers?.[prayerKey]?.time || "--:--";
              ...
              return (
                ...
                >
                  <View className="flex-row items-center flex-1">
                    <View
                      className={`w-10 h-10 rounded-xl items-center justify-center mr-4 ${isPrayed ? "bg-gold/10" : "bg-white/5"}`}
                    >
                      <Ionicons
                    ...
                    <View>
                      <Text
                        className={`font-bold text-base ${isPrayed ? "text-gold" : "text-white"}`}
                      >
                        {prayer}
                      </Text>
                  ...
                  <View className="flex-row items-center">
                    {!isPrayed && (
                      <View className="bg-red-500/10 px-2 py-0.5 rounded-md mr-3 border border-red-500/20">
                        <Text className="text-red-400 text-[8px] font-bold uppercase tracking-tighter">
                          Missed
                        </Text>
                      </View>
                    )}
        ...
        <View className="px-6 pb-12 pt-4">
          ...
          >
            <Ionicons
              name={updating ? "sync" : "save-outline"}
              size={18}
          ...
          >
            <Text className="text-white/40 text-xs font-bold underline">
              Cancel & Discard
            </Text>
          </TouchableOpacity>
        </View>
