...
const VerseCard = ({
  ...
  const handleShare = async () => {
    try {
      await Share.share({
        message: [arabic, transliteration, english]
          .filter(Boolean)
          .join("\n\n"),
      });
    } catch {
  ...
  return (
    ...
    >
      ...
      >
        <Animated.View
          style={[styles.leftAccent, { opacity: accentOpacity }]}
        />

        {isPlaying && (
          <View className="absolute bottom-0 overflow-hidden p-0">
            <LottieView
        ...
        <Text className="text-white/15 text-right text-[10px] leading-7 mt-2">
          Tab to make as Last Read
        </Text>
      </Animated.View>
    </TouchableOpacity>
  );
