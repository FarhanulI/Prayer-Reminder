...
const DropdownSection = ({
  onSelectSurah,
  selectedSurahId,
}: DropdownSectionProps): React.ReactNode => {
  const isFocused = useIsFocused();
  const [surah, setSurah] = useState<DropdownOption>();

  const { data: surahsList, isLoading } = useSurahsLists({
    lang: "en",
    enabled: isFocused,
  });
    ...
    <View className=" bg-emerald-dark border border-white/5 rounded-2xl p-2 mb-4">
      <Dropdown
        ...
        renderTrigger={(value, isOpen, toggleDropdown) => (
          ...
          >
            <Text className="text-[#a5b4a5] text-[10px] font-bold uppercase tracking-widest mb-1">
              Select Surah
            </Text>
            <View className="flex-row items-center">
              {isLoading ? (
                ...
                <Text className="text-gold font-bold mr-2 text-sm">
                  {value}
                </Text>
              )}
              <Feather
